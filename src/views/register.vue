...
export default {
  data() {
    ...
    return {
      imgSrc: require("../assets/motianlun.jpg"),
      dialogVisible1: true,
      ruleForm: {
        name: "",
        pass: "",
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {//先验证是否输入了数据
        if (valid) {
          let that = this;
          //数据库中获取数据
          let url = "http://127.0.0.1:8088/user?name=" + that.ruleForm.name;
